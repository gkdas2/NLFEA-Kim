...
def connectivity3D(numElementsX, numElementsY, numElementsZ):
    ...
    numElements = numElementsX * numElementsY * numElementsZ  # Total number of elements
    numNodes = (
        (numElementsX + 1) * (numElementsY + 1) * (numElementsZ + 1)
    )  # Total number of Elements
    nodeIndex = np.arange(1, numNodes + 1, dtype=np.int64)  # Index of all nodes

    mat = nodeIndex.reshape((numElementsZ + 1, numElementsY + 1, numElementsX + 1))
...
def generate2Dmesh(totalLengthx, totalHeight, numElementsX, numElementsY):
    ...
    eleLength = totalLengthx / numElementsX
...
def generate3Dmesh(
    totalLengthx, totalLengthy, totalLengthz, numElementsX, numElementsY, numElementsZ
):
...
def write2Dmesh(filename, map, coord):
    ...
    for i in range(Nnodes):
        file.write(
            ...
            + str(int(i + 1))
            + "\t , \t"
            + str(round(coord[i, 0], 6))
            + "  \t, \t  "
            + str(round(coord[i, 1], 6))
        )
    file.write("\n \n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A simple script to generate 2D/3D rectangular mesh in Abaqus format"
    )
    parser.add_argument(
        "--DM", type=float, default=2, help="Dimension: 2 = 2D or 3 = 3D"
    )
    ...
    parser.add_argument(
        "--NX", type=int, default=1, help="Number of elements along x axis"
    )
    parser.add_argument(
        "--NY", type=int, default=1, help="Number of elements along y axis"
    )
    parser.add_argument(
        "--NZ", type=int, default=1, help="Number of elements along z axis"
    )
    parser.add_argument(
        "--FILE",
        default="mesh.msh",
        help="Filename with extension to store mesh information",
    )

    a = parser.parse_args()

    DIMENSION, LX, LY, LZ, NX, NY, NZ, FILENAME = (
        a.DM,
        a.LX,
        a.LY,
        a.LZ,
    ...
    if DIMENSION == 3:
        (
            ...
            nodeIndex,
            elIndex,
        ) = generate3Dmesh(LX, LY, LZ, NX, NY, NZ)
